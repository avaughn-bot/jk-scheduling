...
<body>
    ...
    <script type="text/babel">
        ...
        const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzsatCOVQCVz8ZGkzPKetM27csAO4xlXYjs6HiiGu4pl-fmhvSXz_vk_7f9r66ydg0LQQ/exec';
        // Example: 'https://drive.google.com/file/d/1nawrFItA-QFWbInQwY1VE8aDH2m0B1o7/view?usp=sharing'
            ...
            return (
                <div style={styles.container}>
                    ...
                    <div style={styles.header}>
                        <h1 style={styles.title}>📅 John Kasperek Co. Scheduling</h1>
                        ...
                        <div style={styles.deskView}>
                            {Object.entries(offices).map(([officeName, deskList]) => (
                                <div key={officeName} style={styles.officeSection}>
                                    <h3 style={styles.officeTitle}>🏢 {officeName} Office</h3>
                                    ...
                                    <label style={styles.label}>
                                        ...
                                        {deskConflict && (
                                            <span style={styles.conflictWarning}>
                                                ⚠️ Conflict: {deskConflict} has this desk
                                            </span>
                                        )}
                                    </label>
                                    <select
                                        ...
                                        {availableDesks.map(desk => {
                                            ...
                                            return (
                                                <option key={desk} value={desk}>
                                                    {desk} {occupant ? `(⚠️ ${occupant})` : '(Available)'}
                                                </option>
                                            );
                                        })}
                                    </select>
            ...
            useEffect(() => {
                ...
                if (conflictDays.length > 0) {
                    if (conflictDays.length <= 3) {
                        setConflictWarning(`⚠️ Desk conflicts on: ${conflictDays.join(', ')}`);
                    } else {
                        setConflictWarning(`⚠️ ${conflictDays.length} desk conflicts detected. First 3: ${conflictDays.slice(0, 3).join(', ')}`);
                    }
                } else {
                    setConflictWarning('');
                }
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            {formData.dateRangeType === 'custom' && (
                                <div style={styles.formGroup}>
                                    ...
                                    <div style={{fontSize: '12px', color: '#718096', marginTop: '8px'}}>
                                        💡 Perfect for client engagements lasting multiple weeks
                                    </div>
                                </div>
                            )}
                                ...
                                <div style={styles.formGroup}>
                                    ...
                                    {!conflictWarning && (
                                        <div style={{fontSize: '12px', color: '#718096', marginTop: '5px'}}>
                                            💡 Tip: Desk conflicts will be checked automatically as you select options above.
                                        </div>
                                    )}
                                </div>
                            )}
                            ...
                            <div style={styles.bulkSchedulePreview}>
                                <strong>Preview:</strong> {(() => {
                                    ...
                                    return `This will create ${totalAssignments} assignments (${matchingDays} days × ${staffCount} staff) for ${dateRangeText}.`;
                                })()}
                            </div>
